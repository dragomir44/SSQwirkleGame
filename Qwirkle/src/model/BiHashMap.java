...
public class BiHashMap<KEYA, KEYB, V> {

private final Map<KEYA, Map<KEYB, V>> mMap;

public BiHashMap() {
    mMap = new HashMap<KEYA, Map<KEYB, V>>();
}
 ...
 */
public V put(KEYA key1, KEYB key2, V value) {
    Map<KEYB, V> map;
    if (mMap.containsKey(key1)) {
        map = mMap.get(key1);
    } else {
        map = new HashMap<KEYB, V>();
        mMap.put(key1, map);
    }
 ...
 * @see Map#get(Object)
 */
public V get(KEYA key1, KEYB key2) {
    if (mMap.containsKey(key1)) {
 ...
 */
public boolean containsKeys(KEYA key1, KEYB key2) {
